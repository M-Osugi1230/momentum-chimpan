...
この場合も `output/daily_report.xlsx` と `data/momentum_history.csv` が作成・更新されます。ただし、これは検証モードであり、投資判断用の全銘柄スキャンではありません。
...
実行日、アプリ版、レポート形式、JPX上場銘柄数、通常株ユニバース数、除外銘柄数、実スキャン対象銘柄数、取得成功数、取得失敗数、年初来高値更新銘柄数、買い候補数、売り候補数、検証モード、処理時間秒を表示します。
...
Errorsシートに1件以上のエラーがある場合、`data.error_backup_dir` 配下に日時別フォルダを作成し、`errors.csv`, `daily_report.xlsx`, `momentum_history.csv`, `jpx_list_cache.csv` を可能な範囲でコピーします。これにより、後から失敗銘柄や当日のレポート状態を確認できます。
...
1. **まず見るポイント**: 買い候補、売り候補、年初来高値更新、取得失敗をカード形式で表示します。
2. **実行状況**: スキャン対象数、取得成功率、除外銘柄数、処理時間などをまとめて表示します。
3. **スコアの見方**: 100 点満点の配点を短く説明します。
4. **買い候補 上位N件**: 各銘柄をカード形式で表示し、主要指標・「スコア内訳」・「理由」を小見出しで分けて表示します。iPhoneなどのスマートフォンでも、1銘柄ずつ確認しやすい構成です。
5. **売り候補**: `holdings.csv` の保有銘柄のうち、確認対象になった理由を表示します。
6. **エラー・詳細**: 取得失敗の有無と、Excel artifact の確認先を表示します。
...
メール上の「買い候補」「売り候補」は売買指示ではありません。詳細な数値は Excel artifact の `daily_report.xlsx` を確認してください。


### メール改善版の反映確認

最新版では、`main.py` に `MIMEMultipart`, `build_html_email`, `score_ytd_high`, `まず見るポイント`, `スコア内訳` という文言が含まれます。ローカルで更新できているか確認する場合は、以下を実行してください。

```bash
grep -n "MIMEMultipart" main.py
grep -n "build_html_email" main.py
grep -n "score_ytd_high" main.py
grep -n "まず見るポイント" main.py
grep -n "スコア内訳" main.py
```

いずれも行番号が表示されれば、メール改善版が反映されています。
