...
メール本文は、初めて受け取った人でも概要を把握しやすいように、次の順番で表示します。

1. **まず見るポイント**: 買い候補の最上位、売り候補件数、年初来高値更新数、取得失敗数を先に表示します。
2. **サマリー**: スキャン対象数、取得成功率、除外銘柄数、処理時間などの実行状況を表示します。
3. **スコアの見方**: 100 点満点の配点を説明します。
4. **買い候補 上位N件**: 各銘柄について、理由とスコア内訳を表示します。
5. **売り候補**: `holdings.csv` の保有銘柄のうち、確認対象になった理由を表示します。
6. **エラー・注意**: 取得失敗の有無と、詳細確認先を表示します。
...
最新版では、`main.py` に `score_ytd_high`, `まず見るポイント`, `スコア内訳` という文言が含まれます。ローカルで更新できているか確認する場合は、以下を実行してください。
