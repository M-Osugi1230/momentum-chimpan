...
def positive_volume_sessions(prices: pd.DataFrame) -> pd.DataFrame:
    """Return executable observations rather than rejecting a holding for any zero-volume day.

    A zero-volume observation is not an executable session for that stock. The former strict
    implementation rejected the entire 20/40/60-session outcome when a single zero-volume row
    occurred anywhere in the window. That created a severe horizon-dependent illiquidity bias,
    especially in 2018. We now count only positive-volume observations as stock trading sessions,
    matching the ranking and price-path definitions.
    """
